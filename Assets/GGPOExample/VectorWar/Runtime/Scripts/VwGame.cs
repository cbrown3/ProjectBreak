...
namespace VectorWar {
    ...
    public struct VwGame : IGame {
        ...
        public void ParseShipInputs(long inputs, int i, out float heading, out float thrust, out int fire) {
            var ship = _ships[i];
            if (verbose) {
                Debug.Log($"parsing ship {1} inputs: {inputs}.");
            }
        ...
        public void MoveShip(int index, float heading, float thrust, int fire) {
            ...
            if (verbose) {
                Debug.Log($"calculation of new ship coordinates: (thrust:{thrust} heading:{heading}).");
            }
            ...
            if (verbose) {
                Debug.Log($"new ship velocity: (dx:{ship.velocity.x} dy:{ship.velocity.y}).");
            }
            ...
            if (verbose) {
                Debug.Log($"new ship position: (dx:{ship.position.x} dy:{ship.position.y}).");
            }
